...
export const addTask = (task, id) => async (dispatch) => {
	...
	const response = await axios.post('http://localhost:4002/task/add', taskData);
	if (response) {
...
export const getAllTasks = (token) => async (dispatch) => {
    ...
    try {
        const response = await axios.get(
            'http://localhost:4002/task/tasks',
            config
        );
...
export const arrowClick = (item, string) => async () => {
    ...
    try {
        let response = await axios.put(
            `http://localhost:4002/task/${taskData.id}`,
            taskData
        );
...
export const deleteItem = (id) => async (dispatch) => {
    let res = await axios.delete(`http://localhost:4002/task/${id}`);
